...
export default function Changelog() {
  ...
  const getColors = (message: string) => {
    if (message.startsWith("feat"))
      return { bg: "#D9F7E3", border: "#34A853" };
    if (message.startsWith("fix"))
      return { bg: "#FFE8CC", border: "#FB8C00" };
  ...
  return (
    ...
    >
      <Box sx={{ maxWidth: 700, mx: "auto" }}>
        ...
        {changelog.map((entry) => {
          const { bg, border } = getColors(entry.commits[0].message);
          return (
            <Accordion
              key={entry.version}
              sx={{
            ...
            >
              ...
              <AccordionDetails>
                {entry.commits.map((commit) => (
                  <Box key={commit.date} sx={{ mb: 2 }}>
                    <Typography
                      variant="subtitle1"
                      sx={{ fontWeight: 500, color: "#0C100F" }}
                    >
                      {commit.message.split(":").splice(1, 1)}
                    </Typography>
